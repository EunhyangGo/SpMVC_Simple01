...
<section>
	...
	<c:choose>
		...
		<tr>
			...
			<!-- 매출표시 -->
			<c:if test="${VO.io_check=='2'}">매입
			<td></td><td></td>
			<td>${VO.io_price}</td>
			<td class="w3-right">
			<fmt:formatNumber value="${VO.io_price * VO.io_quan}"
			type="currency"/>
			</td>
			</c:if>
		</tr>
...
<!-- 
	 ...
	 fmt formatNumber var="값.값" type="percent" 만약 0.3이면 30% 표현
	 											만약 1.2이면 120%
	 											
  	 fmt formatNumber var="값.12341" pattern=".00" 숫자형으로 표현하되 소수점이하 2자리까지 표시
  	 fmt formatDate pattern="yyyy/mm/dd" : 값이 날짜일 경우 	 											
